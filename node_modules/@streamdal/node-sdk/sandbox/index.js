...
const logPipeline = async (streamdal, audience, input) => {
    ...
    console.debug(`sending pipeline request for ${audience.serviceName} - ${OperationType[audience.operationType].toLowerCase()}`);
...
export const runPipeline = (streamdal, audience, input, interval = 0) => interval
    ? setInterval(() => {
        void logPipeline(streamdal, audience, input);
    }, interval)
    : void logPipeline(streamdal, audience, input);
export const randomPipeline = (streamdal, audience, input) => {
    runPipeline(streamdal, audience, input);
    setTimeout(() => randomPipeline(streamdal, audience, input), Math.floor(Math.random() * (3000 - 250) + 3000));
};
...
export const tailFriendly = async () => {
    ...
    runPipeline(streamdalA, { ...audienceAConsumer, operationName: "kafka-consumer-two" }, exampleData);
    runPipeline(streamdalA, { ...audienceAConsumer, operationName: "kafka-consumer-three" }, exampleData);
    runPipeline(streamdalA, { ...audienceAConsumer, componentName: "another-kafka" }, exampleData);
    ...
    runPipeline(streamdalB, { ...audienceBProducer, componentName: "kafka" }, exampleData);
};
export const highVolumeTail = async () => {
    const streamdalA = await registerStreamdal(serviceAConfig);
    const streamdalB = await registerStreamdal(serviceBConfig);
    runPipeline(streamdalA, audienceAConsumer, exampleData, 500);
    runPipeline(streamdalA, { ...audienceAConsumer, operationName: "kafka-consumer-two" }, exampleData, 500);
    runPipeline(streamdalA, { ...audienceAConsumer, operationName: "kafka-consumer-three" }, exampleData, 1000);
    runPipeline(streamdalA, { ...audienceAConsumer, componentName: "another-kafka" }, exampleData, 500);
    runPipeline(streamdalA, { ...audienceAConsumer, componentName: "another-kafka" }, exampleData);
    ...
    runPipeline(streamdalB, { ...audienceBProducer, componentName: "kafka" }, exampleData, 500);
};
export const throughputFriendly = async () => {
    const streamdalA = await registerStreamdal(serviceAConfig);
    const streamdalB = await registerStreamdal(serviceBConfig);
    randomPipeline(streamdalA, audienceAConsumer, exampleData);
    randomPipeline(streamdalA, { ...audienceAConsumer, operationName: "kafka-consumer-two" }, exampleData);
    randomPipeline(streamdalA, { ...audienceAConsumer, operationName: "kafka-consumer-three" }, exampleData);
    randomPipeline(streamdalA, { ...audienceAConsumer, componentName: "another-kafka" }, exampleData);
    ...
    randomPipeline(streamdalB, { ...audienceBProducer, componentName: "kafka" }, exampleData);
};
...
export const exampleMultipleGroup = async () => {
    ...
    runPipeline(streamdalB, audienceBConsumer, exampleData);
    runPipeline(streamdalB, audienceBProducer, exampleData);
    runPipeline(streamdalB, { ...audienceBProducer, operationName: "kafka-producer-two" }, exampleData);
};
...
export const exampleStaggeredMultipleComponentsPerServiceAndPerGroup = async () => {
    const streamdalA = await registerStreamdal(serviceAConfig);
    const streamdalB = await registerStreamdal(serviceBConfig);
    const streamdalC = await registerStreamdal(serviceCConfig);
    runPipeline(streamdalA, audienceAConsumer, exampleData, 2000);
    runPipeline(streamdalA, { ...audienceAConsumer, operationName: "kafka-consumer-two" }, exampleData, 4000);
    runPipeline(streamdalA, { ...audienceAConsumer, operationName: "kafka-consumer-three" }, exampleData, 6000);
    runPipeline(streamdalA, { ...audienceAConsumer, componentName: "another-kafka" }, exampleData, 8000);
    ...
    runPipeline(streamdalB, { ...audienceBProducer, componentName: "kafka" }, exampleData, 16000);
